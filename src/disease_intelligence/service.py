...
class DiseaseIntelligenceService:
    ...
    async def populate_disease(
        ...
        try:
            # Phase 1: Search for all data types
            logger.info("Phase 1: Multi-source literature search")
            prevalence_papers, treatment_papers, failure_papers = await asyncio.gather(
                self._search_prevalence(disease_name),
                self._search_treatment(disease_name),
                self._search_failure_rates(disease_name),
            )
            ...
            prevalence_data, treatment_data, failure_data = await asyncio.gather(
                self._extract_prevalence(disease_name, prevalence_papers),
                self._extract_treatment(disease_name, treatment_papers),
    ...
    async def _search_prevalence(self, disease: str) -> List[Paper]:
        """Search multiple sources for prevalence data."""
        papers = []

        # PubMed search - expanded queries
        pubmed_queries = [
            # Original queries
            f"{disease} prevalence United States epidemiology",
            f"{disease} incidence United States population",
            f"{disease} burden of disease epidemiology",
            # More specific sources
            f"{disease} national estimates United States",
        ...
        for query in pubmed_queries:
            try:
                results = await self._search_pubmed(query, max_results=8)
                papers.extend(results)
            except Exception as e:
                logger.warning(f"PubMed search failed for '{query}': {e}")

        # Web search for authoritative sources - expanded
        web_queries = [
        ...
        for query in web_queries:
            try:
                results = await self._search_web(query, max_results=4)
                papers.extend(results)
            except Exception as e:
        ...
        return unique_papers[:15]  # Limit to top 15

    async def _search_treatment(self, disease: str) -> List[Paper]:
        """Search for treatment guidelines and patterns."""
        papers = []

        # PubMed for guidelines
        pubmed_queries = [
            f"{disease} treatment guidelines recommendations",
            f"{disease} standard of care therapy",
            f"{disease} treatment algorithm biologic",
        ]
    ...
    async def _search_failure_rates(self, disease: str) -> List[Paper]:
        """Search for treatment failure/inadequate response data."""
        papers = []

        pubmed_queries = [
            # Original queries
            f"{disease} inadequate response first line therapy",
            f"{disease} treatment failure conventional",
            f"{disease} biologic naive population",
            f"{disease} real world treatment discontinuation",
    ...
    def _assemble_disease(
        ...
        # Build per-source prevalence estimates with new fields
        ...
        segmentation = PatientSegmentation(
            pct_treated=seg_raw.get("pct_treated"),
            severity=severity,
        )

        # Build per-source failure rate estimates with new fields
        failure_estimates = []
        ...
        failure_rates = FailureRates(
            ...
            source=failure_estimates[0].title if failure_estimates else None,
            # Switch vs discontinuation
...
def create_disease_intelligence_service(db) -> DiseaseIntelligenceService:
    ...
    from src.tools.pubmed import PubMedSearcher
    from src.tools.semantic_scholar import SemanticScholarSearcher
