...
class ApprovalDateExtractor:
    ...
    def _fuzzy_match_indication(
        ...
        # Tier 4: Dynamic keyword match
        # Extract significant words from DB indications and match against scraped indication
        # This works for any therapeutic area without hardcoded keywords
        matches = []
        
        # Common stop words to ignore
        stop_words = {'with', 'and', 'or', 'in', 'of', 'the', 'for', 'to', 'a', 'an', 'as', 'at', 'by', 'from'}
        
        for db_ind in db_indications:
            # Extract significant words (2+ chars, not stop words)
            db_words = set(word for word in db_ind.split() if len(word) >= 2 and word not in stop_words)
            scraped_words = set(word for word in scraped_indication.split() if len(word) >= 2 and word not in stop_words)
            
            # Check for word overlap
            common_words = db_words.intersection(scraped_words)
            
            # Match if there are 2+ common significant words
            if len(common_words) >= 2:
                matches.append(db_ind)
                logger.debug(f"  Tier 4: '{db_ind}' matched via words: {common_words}")
